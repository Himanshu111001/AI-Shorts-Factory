...
def run_tests():
    ...
    try:
        ...
        print("\n[Test 4] Verify update 30, 'Text Generated'")
        job_service.update_progress(job_uuid, 30, "Text Generated")
        res = client.get(f"/jobs/{job_id}")
        data = res.json()
        if data["progress"] != 30: raise AssertionError("Progress not 30")
        if data["current_step"] != "Text Generated": raise AssertionError("Wrong step")
        
        print("\n[Test 5] Verify update 35, 'Generating Audio'")
        job_service.update_progress(job_uuid, 35, "Generating Audio")
        res = client.get(f"/jobs/{job_id}")
        data = res.json()
        ...
        print("\n[Test 6] Verify update 50, 'Audio Generated'")
        job_service.update_progress(job_uuid, 50, "Audio Generated")
        res = client.get(f"/jobs/{job_id}")
        data = res.json()
        if data["progress"] != 50: raise AssertionError("Progress not 50")
        if data["current_step"] != "Audio Generated": raise AssertionError("Wrong step")
        
        print("\n[Test 7] Verify update 55, 'Generating Images'")
        job_service.update_progress(job_uuid, 55, "Generating Images")
        res = client.get(f"/jobs/{job_id}")
        data = res.json()
        if data["progress"] != 55: raise AssertionError("Progress not 55")
        if data["current_step"] != "Generating Images": raise AssertionError("Wrong step")
        
        print("\n[Test 8] Verify update 75, 'Images Generated'")
        job_service.update_progress(job_uuid, 75, "Images Generated")
        res = client.get(f"/jobs/{job_id}")
        data = res.json()
        if data["progress"] != 75: raise AssertionError("Progress not 75")
        if data["current_step"] != "Images Generated": raise AssertionError("Wrong step")

        print("\n[Test 8b] Verify update 80, 'Rendering Video'")
        job_service.update_progress(job_uuid, 80, "Rendering Video")
        res = client.get(f"/jobs/{job_id}")
        data = res.json()
        if data["progress"] != 80: raise AssertionError("Progress not 80")
        if data["current_step"] != "Rendering Video": raise AssertionError("Wrong step")

        print("\n[Test 8c] Verify update 95, 'Video Rendered'")
        job_service.update_progress(job_uuid, 95, "Video Rendered")
        res = client.get(f"/jobs/{job_id}")
        data = res.json()
        if data["progress"] != 95: raise AssertionError("Progress not 95")
        if data["current_step"] != "Video Rendered": raise AssertionError("Wrong step")
